...
/**
 * The Piloting skill is levelled through piloting flying entities like ghasts.
 */
public class Piloting extends AbstractMovementSkill {
    ...
    public MovementSkillType getSkillType() {
        return MovementSkillType.PILOTING;
    }
    ...
    public void executeReward(@NotNull Player player, Object... skillData) {
        ...
        if (!(player.getVehicle() instanceof Flying)) return;
        final Flying vehicle = (Flying) player.getVehicle();
        if (vehicle.hasPotionEffect(PotionEffectType.SPEED)) vehicle.removePotionEffect(PotionEffectType.SPEED);
        vehicle.addPotionEffect(new PotionEffect(PotionEffectType.SPEED, 600, 2, true, false));
        player.playSound(player.getLocation(), Sound.ENTITY_GHAST_AMBIENT, 5, 2);
        player.sendMessage(messageService.convert(Objects.requireNonNull(messageService.getlang().getString("Skills.Piloting"))));
    }
