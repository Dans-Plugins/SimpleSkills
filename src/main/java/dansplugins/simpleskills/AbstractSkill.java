...
public abstract class AbstractSkill implements Listener {
     ...
     */
    @SafeVarargs
    public AbstractSkill(@NotNull String name, @NotNull Class<? extends Event>... triggers) {
        ...
        this.name = name;
        this.active = true;
        final LocalConfigService configService = LocalConfigService.getInstance();
        this.expReq = configService.getconfig().getInt("defaultBaseExperienceRequirement", 10);
        this.expFactor = configService.getconfig().getDouble("defaultDefaultExperienceIncreaseFactor", 1.2);
     ...
     * @see #AbstractSkill(String, Class[])
     */
    ...
    public PlayerRecord getRecord(@NotNull Player player) {
        return PersistentData.getInstance().getPlayerRecord(player.getUniqueId());
    }
    ...
    public void register() {
        ...
        for (Triggers value : Triggers.values()) {
            Bukkit.getPluginManager().registerEvent(
                    value.getTriggerClass(), this, EventPriority.MONITOR, executor, SimpleSkills.getInstance()
            );
        }
    }
